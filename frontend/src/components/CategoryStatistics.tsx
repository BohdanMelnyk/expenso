...
const CategoryStatistics: React.FC = () => {
  ...
  const getVendorData = (): VendorData[] => {
    const vendorMap = new Map<number, { id: number; name: string; amount: number; count: number }>();
    const totalAmount = expenses.reduce((sum, exp) => sum + exp.amount, 0);
    
    expenses.forEach(expense => {
      const vendorId = expense.vendor?.id || 0;
      const vendorName = expense.vendor?.name || 'Unknown';
      
      if (!vendorMap.has(vendorId)) {
        vendorMap.set(vendorId, { id: vendorId, name: vendorName, amount: 0, count: 0 });
      }
  ...
  const handleVendorClick = (vendorId: number) => {
    navigate(`/vendor/${vendorId}?period=${selectedTimeFrame}`);
  };
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {vendorData.map((vendor, index) => (
              <button
                ...
                className="flex justify-between items-center p-3 border rounded-lg w-full hover:bg-gray-50 hover:border-blue-300 transition-colors cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
              >
                <div className="flex items-center">
                  <div 
